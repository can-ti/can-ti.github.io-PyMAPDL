...
本示例演示如何用 Python 创建一个图形用户界面 (GUI) 应用程序，使用 PyMAPDL 计算梁的挠度。

Simulation setup
================

以下脚本使用 PySide6 启动图形应用程序：
...
**前处理** 选项卡包含泊松比、杨氏模量、梁长和模拟节点数的输入字段。
...
首先从 ``pyvistaqt`` 包中导入 `QtInteractor <https://qtdocs.pyvista.org/api_reference.html#qtinteractor>`_ 类，并从 ``ansys-mapdl-core`` 包中导入 :class:`MapdlTheme <ansys.mapdl.core.theme.MapdlTheme>` 类：
...
在 windows 中启动 MAPDL 实例
=======================================

在 MainWindow 中为 MAPDL 实例添加一个属性，并导入 ``launch_mapdl`` 软件包。
